...
def history(request, info):
    if info == "people":
        ...
        return HttpResponse(f"""Население в 1979 году составляло 650 000 человек. По переписи населения 1989 года 
        составляло 723 000 человек, по переписи 2001 года составило 669 000 человек. По состоянию на 1 декабря 2015
         года в городе проживало 641 274 постоянных жителей и 642 788 человек наличного населения. Население на 1
          декабря 2016 года — 638 395 постоянных жителей и 639 876 человек наличного населения. Население на 1 
          ноября 2017 года — 631 647 постоянных жителей и 633 128 человек наличного населения. По состоянию 1 
          января 2021 года в Кривом Роге насчитывалось 615,5 тысяч 
          жителей.<br><div><h2><a href='http://127.0.0.1:8000/'>Главная страница</a></h2></div>""")

def history_cities(request, cities, info):
    if cities == "germany":
        if info.isdigit():
            if info == "1939":
                return HttpResponse(f"""Германия напала на Польшу""")
            elif info == "1945":
                return HttpResponse(f"""Германия проиграла вторую мировую войну""")
            else:
                return HttpResponse(f"""<h1>Нужно загуглить, что происходило в Германии в {info} году</h1>""")
        else:
            return HttpResponse(f"""Нужно ввести дату, а не \"{info}\"""")
    else:
